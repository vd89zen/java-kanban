        ...
        try {
            if (requestMethod.equals("DELETE")) {
                if (splitPath.length == 3) {
                    try {
                        ...
                        resultOfOperation = taskManager.removeSubtaskByIdNumber(id);
                    ...
                    textForGson.setRequest(requestMethod + " " + httpExchange.getRequestURI());
                    textForGson.setDetail("For DELETE path must be '/subtasks/{id}'");
                    sendBadRequest(httpExchange, gson.toJson(textForGson));
                }
                return;
            }
            ...
            if (requestMethod.equals("POST")) {
                ...
                if (splitPath.length == 2) {
                    ...
                    textForGson.setRequest(requestMethod + " " + httpExchange.getRequestURI());
                    textForGson.setDetail("For POST path must be '/subtasks'");
                    sendBadRequest(httpExchange, gson.toJson(textForGson));
                }
                return;
            }
            ...
            if (requestMethod.equals("GET")) {
                if (splitPath.length == 2) {
                    Optional<ArrayList<Subtask>> allSubtasks = taskManager.getAllSubtasks();
                    if (allSubtasks.isEmpty()) {
                        textForGson.setDetail("List is empty");
                        sendNotFound(httpExchange, gson.toJson(textForGson));
                    } else {
                        sendText(httpExchange, gson.toJson(allSubtasks.get()));
                    }
                } else if (splitPath.length == 3) {
                    try {
                        int id = Integer.valueOf(splitPath[2]);
                        if (id > 0) {
                            Optional<Subtask> subtask = taskManager.getSubtaskByIdNumber(id);
                            if (subtask.isEmpty()) {
                                textForGson.setTaskType("subtask");
                                textForGson.setTaskIdNumber(id);
                    ...
                    textForGson.setRequest(requestMethod + " " + httpExchange.getRequestURI());
                    textForGson.setDetail("For GET path must be '/subtasks' or '/subtasks/{id}'");
                    sendBadRequest(httpExchange, gson.toJson(textForGson));
                }
                return;
            }
